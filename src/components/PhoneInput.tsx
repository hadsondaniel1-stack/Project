  ...
  useEffect(() => {
    const detectCountryFromIP = async () => {
      try {
        ...
        // Silently fail - keep default country (US)
      }
    };
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            role="combobox"
            aria-expanded={open}
            className="w-[140px] justify-between bg-slate-800 border-slate-700 text-white hover:bg-slate-700"
          >
              ...
              <CommandGroup className="max-h-[300px] overflow-y-auto">
                {countries.map((country) => (
                  ...
                  >
                    ...
                    <span className="text-slate-400 text-sm">{country.dialCode}</span>
                  </CommandItem>
                ))}
              </CommandGroup>
            </CommandList>
